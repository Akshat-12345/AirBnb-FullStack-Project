...
// Helper: Sanitize redirect destination to prevent auth loops & 500 Bad Request errors
const getSafeRedirectUrl = (rawUrl) => {
  if (
    !rawUrl ||
    typeof rawUrl !== 'string' ||
    rawUrl.includes('/auth') ||
    rawUrl.includes('/login') ||
    rawUrl.includes('/signup') ||
...
module.exports.Loginuser = async (req, res) => {
  ...
  const targetUrl = getSafeRedirectUrl(res.locals.redirectUrl);
  if (req.session) {
...
module.exports.googleLoginCallback = (req, res) => {
  ...
  // Prevent loop back to OAuth/Login paths
  const targetUrl = getSafeRedirectUrl(res.locals.redirectUrl);
  if (req.session) {
    delete req.session.redirectUrl;
  }
